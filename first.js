...
//function for modal
function createModal(imgUrl, name, height, width, depth, weight, reviews, price, inStock, color, os, color, chip){
...
function addToCart(imgUrl, name, price){
    ...
    countDiv.append(countDivLeftArrow, countDivCounter, countDivRightArrow, countDivCloser);

    const totalAmount = document.querySelector('.cart-results-amount-size');
    totalAmount.innerHTML = Number(countDivCounter.innerHTML) + Number(totalAmount.innerHTML);
    ...
    countDivCloser.addEventListener('click', function(e){
        ...
        totalPrice.innerHTML = Number(totalPrice.innerHTML) - (price * countDivCounter.innerHTML);
...
// add items
for(let i of items){
    adder(i.name, i.orderInfo.inStock, i.imgUrl, i.price, i.orderInfo.reviews, i.id);
}
...
for(let i of items){
    ...
    ourCard.addEventListener('click', function(e){
        ...
        if(e.target == currentCardNumber){
            ...
            createModal(i.imgUrl, i.name, i.size.height, i.size.width, i.size.depth, 
                i.size.weight, i.orderInfo.reviews, i.price, i.orderInfo.inStock, 
                i.color, i.os, i.color);
        }
    })
}
...
for(let i of itemsColorsArr){
    ...
    let divForLeftColumnColors = document.querySelector('.divForLeftColumnColors');
    let divForRightColumnColors = document.querySelector('.divForRightColumnColors');
...
const dataOfSecondLevelMemory = document.querySelector(".secondLevelOfMemory");
const dataOfSecondLevelOs = document.querySelector(".secondLevelOfOs");
const dataOfSecondLevelDisplay = document.querySelector(".secondLevelOfDisplay");

const filteredSecondLevelPrice = 
secondLevelInputFrom.value.length > 0 && secondLevelInputTo.value.length > 0 ?
items.filter((i) => {
  if (i.price > Number(secondLevelInputFrom.value) && i.price < Number(secondLevelInputTo.value)) {
    return true
  }
...
const selectedMemory = [
    ...dataOfSecondLevelMemory.querySelectorAll(".secondLevelOfMemory .secondLevelCheck:checked"),
...
const selectedOs = [
    ...dataOfSecondLevelOs.querySelectorAll(".secondLevelOfOs .secondLevelCheck:checked"),
  ].map((n) => n.value);
...
...dataOfSecondLevelDisplay.querySelectorAll(".secondLevelOfDisplay .secondLevelCheck:checked"),
